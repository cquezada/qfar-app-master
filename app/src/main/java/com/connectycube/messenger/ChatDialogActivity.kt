...
class ChatDialogActivity : BaseChatActivity(), ChatDialogAdapter.ChatDialogAdapterCallback {
    ...
    private var lastChatDialogId: String? = null
    ...
    private fun updateDialogAdapter(listChats: List<ConnectycubeChatDialog>?) {
        val sortedChats = listChats?.sortedByDescending { it.createdAt }
        /*chatDialogAdapter.submitList(sortedChats)
        if (sortedChats?.isNotEmpty() == true) {
            val lastChatDialog = sortedChats.last()
            onChatDialogSelected(lastChatDialog)
        }*/

        // Filtrar la lista de chats utilizando el dialogId
        val filteredChats = sortedChats?.filter { it.dialogId == currentDialogId }

        chatDialogAdapter.submitList(filteredChats)
        if (filteredChats?.isNotEmpty() == true) {
            val lastChatDialog = filteredChats.last()
            onChatDialogSelected(lastChatDialog)
        }

        //chatDialogAdapter.submitList(listChats)
        /*if (listChats?.isNotEmpty() == true) {
            val lastChatDialog = listChats.last()
            onChatDialogSelected(lastChatDialog)
        }*/
    }
    ...
    private fun startChatActivity(chat: ConnectycubeChatDialog) {
        val intent = Intent(this, ChatMessageActivity::class.java).apply {
            ...
            putExtra("dialogid", currentDialogId)
        }
        startActivityForResult(intent, REQUEST_CHAT_DIALOG_ID)
        overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left)
    }
